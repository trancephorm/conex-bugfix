  ...
  for(const section of $('.section')) {
    ...
    const deleteContainerHandler = function(e, force) {
      ...
      browser.tabs.query({cookieStoreId: cookieStoreId}).then(tabs => {
        ...
        console.error('Error querying tabs for container deletion:', error);
        alert('Error: Could not verify container tabs. Please try again.');
      });
    };
    ...
    const deleteContainerWithTabs = async function(dataset) {
      ...
      try {
        ...
        if (containerTabs.length === allTabs.length) {
          ...
          alert('Error: Safety check failed - would close all tabs. Please try again.');
          return;
        }
        
        // Additional validation: Ensure we have tabs to close and they belong to the container
        if (containerTabs.length === 0) {
          console.log('No tabs found for container:', cookieStoreId);
          deleteContainer(cookieStoreId, dataset.name);
          return;
        }
        
        // Verify all tabs actually belong to the container
        const invalidTabs = containerTabs.filter(tab => tab.cookieStoreId !== cookieStoreId);
        if (invalidTabs.length > 0) {
          console.error('Found tabs that do not belong to the container:', invalidTabs);
          alert('Error: Found tabs that do not belong to this container. Please try again.');
          return;
        }
        ...
        console.error('Error in deleteContainerWithTabs:', error);
        alert('Error: Could not delete container tabs. Please try again.');
      }
    }
